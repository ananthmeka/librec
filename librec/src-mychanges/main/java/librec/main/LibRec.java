...
public class LibRec {
	...
	protected void execute(String[] args) throws Exception {
		...
		for (String config : configFiles) {
			...
			Logs.debug("ANANTH  : librec execute() BEFORE RUN ");
			run();
			Logs.debug("ANANTH  : librec execute() AFTER  RUN ");
		}
	...
	protected void run() throws Exception {
		...
		Logs.debug("ANANTH  : BEFORE REPEAT  ");
		// evaluation setup
		...
		Logs.debug("ANANTH  : AFTER REPEAT  ");
	}
	...
	private void runAlgorithm(LineConfiger evalOptions) throws Exception {
		...
		Logs.debug("ANANTH  : BEFORE printEvalInfo  ");
		printEvalInfo(algo, algo.measures);
		Logs.debug("ANANTH  : AFTER printEvalInfo  ");
	}
	...
	private void printEvalInfo(Recommender algo, AvgMetricCollection ms) throws Exception {
		...
		Logs.debug("ANANTH  : IN printEvalInfo : BEGIN    ");
		...
		Logs.debug("ANANTH  : IN printEvalInfo : END    ");
	}
	...
	Logs.debug("ANANTH  : IN printEvalInfo 2  : BEGIN   ");
        String result = ms.getEvalResultString();
        // we add quota symbol to indicate the textual format of time
	...
	Logs.debug("ANANTH  : IN printEvalInfo 2  : config {}   ", config);
	Logs.debug("ANANTH  : IN printEvalInfo 2  : algoName {}   ", algo.algoName);
	...
	Logs.debug("ANANTH  : IN printEvalInfo 2  : END   ");
    }
